...
module.exports = {
    ...
    create: function(context) {
        ...
        const missingClosingBraceNewLineTab = 'Components with multiple properties and/or wrapped in body tag should have closing brace on new line'
        const missingClosingParenNewLine = 'Closing parentheses for render element should be on a new line'
        const missingLeadingNewLineTab = 'Components with multiple properties and/or wrapped in body tag should start on a new line'
        const multPropsInvalidFormat = 'Rendering a component with multiple properties should format component and properties on individual lines'
        ...
        function _closingBodyTagCheck(node, value) {
            ...
            // TODO: Add check for closing template block tag if it has body tags as well
        ...
        function _endingNewLineTabCheck(node, value) {
            ...
            const hasBodyTags =  _bodyTagCheck(value)
            const hasValidClosingBrace = hasBodyTags ? value.match(/\n\s{12}}}/) : value.match(/\n\s{8}}}/)

            if (!hasValidClosingBrace && expectedPropCount > 1) _throwError(missingClosingBraceNewLineTab)

            // Closing }} looks gook, check to see if any template blocks have closing elements
            return _clostingTemplateBlockCheck(node, value)
        }
        ...
        function _leadingNewLineTabCheck(node, value) {
            ...
            if (hasBodyTags) {
                ...
                const innerContentEnd = innerContentStart[1] ? innerContentStart[1].split('<\/body>') : innerContentStart[0].split('<\/body>')
            ...
            if (leadingNewLineTab) {
                ...
                //TODO: Check for leading new line tab on closing template block tag as well

                // Compoment leading new line/tab check complete, move on to check properties
                return _propertyLineTabCheck(node, value)
            }
        }
        ...
        function _propertyLineTabCheck(node, value) {
            ...
            // Properties look ok, check for ending new line/tabs
            return _endingNewLineTabCheck(node, value)
        }
        ...
        // Public
        //----------------------------------------------------------------------
        // TODO: Replace incoming leading tabs with spaces so we can evaluate both
        return {
            'CallExpression': function(node) {
                ...
                if (node.callee.object && node.callee.object.type === 'ThisExpression') {
                    if (node.callee.property && node.callee.property.name === 'render') {
                        ...
                        node.arguments.forEach(function(TaggedTemplateExpression) {
                            ...
                            if (TaggedTemplateExpression.tag.name === 'hbs') {
                                ...
                                TaggedTemplateExpression.quasi.quasis.forEach(function(quasi) {
